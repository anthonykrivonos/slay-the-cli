...
// A compiled binary reports argv[1] as /$bunfs/root/<name>; from source it keeps
// the .ts path, and then the runner matters: bun runs TS directly, Node needs
// tsx (plain node cannot resolve this tree's extensionless imports).
function invokedAs(): string {
  const p = process.argv[1] ?? "";
  if (!p.endsWith(".ts")) return p.split(/[/\\]/).pop() || "slay";
  const onBun = typeof (globalThis as { Bun?: unknown }).Bun !== "undefined";
  return onBun ? "bun src/cli/main.ts" : "tsx src/cli/main.ts";
}
